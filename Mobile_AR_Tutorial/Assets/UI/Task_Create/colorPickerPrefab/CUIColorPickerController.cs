...
public class CUIColorPickerController : MonoBehaviour
{
    //private TextMeshProUGUI valueColor;    
    private Action<Color> actionColor;
    //private Action<Color> actionSaveColor;
    ...
    private void Setup(Color inputColor)
    {
        ...
        Action applySaturationValue = () =>
        {
            ...
            // Chama actionColor apenas se a cor mudou
            if (_color != resultColor)
            {
                _color = resultColor; // Atualiza a cor interna
                ...
                // Atualiza o valor da cor no TextMeshProUGUI
                //valueColor.text = ColorToHex(resultColor);
                actionColor?.Invoke(resultColor); // Chama actionColor apenas quando necessário
            }
        };
        applyHue();
    ...
    // Fun��o para definir uma nova cor
    public void SetColor(Color newColor)
    {
        _color = newColor;
        Setup(newColor);
        //valueColor.text = "" + ColorToHex(newColor);
    }

    void Awake()
    {
        //valueColor = this.gameObject.GetComponentInChildren<TextMeshProUGUI>();
        /*btnSave = this.gameObject.GetComponentInChildren<Button>();
        btnSave.onClick.AddListener(() => {actionSaveColor.Invoke(_color);});*/
        //Color = Color.gray;
    }
    ...
    void Update()
    {
        if (_update != null)
        {
            _update(); // Chama a função atual de atualização se estiver definida
        }
    }

    public void constructorModalBtnColorObject(Color color, Action<Color> colorAction)
    {
        this.actionColor = colorAction;
        SetColor(color); // Inicializa a cor com a função SetColor
    }
}
