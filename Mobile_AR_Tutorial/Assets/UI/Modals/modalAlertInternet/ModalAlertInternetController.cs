...
public class ModalAlertInternetController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        btn_No.onClick.AddListener(() => Application.Quit());

        btn_Yes = this.gameObject.GetComponentsInChildren<Button>()
                        .FirstOrDefault(txt => txt.name == "btn_Yes");
        btn_Yes.onClick.AddListener(TryNewInternetConnection);
    }

    private void TryNewInternetConnection()
    {
